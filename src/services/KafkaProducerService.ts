...
export class KafkaProducerService {
   ...
   */
  async sendMessage(topic: string, message: string | object, key?: string): Promise<boolean> {
    try {
   ...
   * @param key - Optional message key
   */
  async sendToFlowTopic(orgUsrNode: string, message: string | object, key?: string): Promise<boolean> {
   ...
   */
  async sendBatch(topic: string, messages: Array<{ value: string | object; key?: string }>): Promise<boolean> {
    try {
      const producer = await this.getProducer();
