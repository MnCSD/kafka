...
export class KafkaConsumerService extends EventEmitter {
  ...
  private async getConsumer(): Promise<Consumer> {
    if (!this.consumer) {
      this.consumer = this.kafka.consumer({
        groupId: this.config.groupId + '_' + Math.random().toString(36).substring(7)
      });
   ...
   * @param fromBeginning - Whether to read from the beginning of the topic
   */
  async subscribeToFlowTopics(orgUsrNode?: string, fromBeginning: boolean = false): Promise<void> {
